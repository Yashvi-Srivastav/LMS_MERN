...
export const getCourse = async(req,res)=>{
    ...
    `
       ...
       const aiText = result?.response?.candidates?.[0]?.content?.parts?.[0]?.text?.trim().replace(/[`"\n]/g, "") || "";

       console.log("search", search)
       console.log("Ai text", aiText)

       let searchTerm = (aiText || search).toLowerCase();


if (searchTerm.includes("mobile")) {
  searchTerm = "mobile";
}
if (searchTerm.includes("mern")) {
  searchTerm = "mern";
}
if (searchTerm.includes("artificial")) {
  searchTerm = "artificial";
}
if (searchTerm.includes("devops")) {
  searchTerm = "devops";
}
